...
class TransparentPath(os.PathLike):  # noqa : F811
    ...
    **Warnings about GCS behaviour**
    ...
    """To translate method args and kwargs between file systems"""
    ...
    def set_global_fs(
        ...
        ----------
        fs: str
            'gcs' will use GCSFileSystem, 'aws' or 's3' will use S3FileSystem, 'scw' will use S3FileSystem
            with endpoint 'TransparentPath.scaleway_endpoint_url', 'local' will use LocalFileSystem
        bucket: str
            The bucket name, only valid if using remote (Default value =  None)
        ...
        token: Optional[Union[dict, str]]
            credentials (default value = None). If not specified, will use envvar <GOOGLE/S3>_APPLICATION_CREDENTIALS.
            If not specified either, will try to login with default account, which will work if using a remote machine
            (VM, cluster...)
        ...
        if not (
            ...
            raise ValueError(f"Unknown value {fs} for parameter 'fs_kind'")
        ...
        if fs == "scw":
            get_fs(fs, bucket, token, endpoint_url=TransparentPath.scaleway_endpoint_url)
        else:
            get_fs(fs, bucket, token)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        fs: Optional[str]
            The file system to use, 'local' or 'gcs', 'gs', 'aws', 's3', or 'scw'.
             If None, uses the default one set by
        ...
        token: Optional[Union[dict, str]]
            The path to google application credentials json file to use, if envvar <GOOGLE/S3>_APPLICATION_CREDENTIALS
            is not set and the code is not running on a remote machine.
        nocheck: bool
        ...
        # I never remember whether I should use fs_kind='local' or fs='local'. That way I don't need to.
        if fs is None:
            fs = ""
    ...
    def exists(self) -> bool:
        if str(self.path) == "/" and self.fs_kind == "local":
            return True
        elif self.path == "gs://" or self.path == "s3://":
            return True
        updated = False
    ...
    def url(self) -> Union[None, str]:
        """Returns a clickable link to open the path
